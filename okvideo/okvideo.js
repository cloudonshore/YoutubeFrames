    ...
    yt: {
        ready: function(event){
            ...
            if(options.endtime == 0)
			 {
			   options.endtime = event.target.getDuration();
			 }
			// event.target.playVideo();
			event.target.seekTo(options.starttime,true);
			setInterval(listenforupdate, 500);
		    done = true;
        },
...
function listenforupdate() {
	    if(player.getCurrentTime()>options.endtime)
		{
		  player.seekTo(options.starttime);
		}
      }